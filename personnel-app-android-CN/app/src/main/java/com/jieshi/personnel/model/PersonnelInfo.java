...
public class PersonnelInfo {
    ...
    public String getFullPosition() {
        ...
        if (governmentLevel != null) {
            ...
            String position = governmentLevel.getPosition();
            if (position != null && !position.isEmpty()) {
                // 检查是否是数字（如 999），如果是则使用 department
                if (position.matches("\\d+")) {
                    String dept = governmentLevel.getDepartment();
        ...
        if (villageLevel != null) {
            if (sb.length() > 0) sb.append(" / ");
            String villageName = villageLevel.getVillageName();
            String position = villageLevel.getPosition();
            
            // 检查 position 是否是数字（如 999），如果是则使用默认职务
            boolean isNumericPosition = position != null && position.matches("\\d+");
            
            if (villageName != null && !villageName.isEmpty()) {
                sb.append(villageName);
                if (position != null && !position.isEmpty() && !isNumericPosition) {
            ...
            } else if (position != null && !position.isEmpty() && !isNumericPosition) {
                sb.append(position);
            } else if (isVillageCadre()) {
                sb.append("村干部");
            } else if (isGridDefender()) {
        ...
        // 如果都没有，显示所属村社区
        if (sb.length() == 0 && villageCommunity != null && !villageCommunity.isEmpty()) {
            sb.append(villageCommunity);
        }
