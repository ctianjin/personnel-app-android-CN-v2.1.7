 ...
 * 展示人员全维度信息，支持多身份展示
 * 根据人员类型显示不同的详情卡片：
 * - 镇府干部：4 个卡片（个人简历、家庭成员、个人奖惩、简要评价）
 * - 村两委干部：2 个卡片（个人简历、简要评价）
 * - 网格联防员：仅基本信息
 * 
 * @author 秋天 · 严谨专业版
 * @version 2.1.7
...
public class DetailActivity extends AppCompatActivity {
    ...
    private String personnelType; // "town_official", "village_cadre", "grid_defender"
    ...
    private void setupDetailCards() {
        ...
        if ("town_official".equals(personnelType)) {
            // 镇府干部：显示所有 4 个卡片
            tvWorkExperiences.setVisibility(View.VISIBLE);
            tvAwards.setVisibility(View.VISIBLE);
            tvFamilyMembers.setVisibility(View.VISIBLE);
            tvComment.setVisibility(View.VISIBLE);
        } else if ("village_cadre".equals(personnelType)) {
            // 村两委干部：只显示个人简历和简要评价
            tvWorkExperiences.setVisibility(View.VISIBLE);
            tvAwards.setVisibility(View.GONE);
            tvFamilyMembers.setVisibility(View.GONE);
            tvComment.setVisibility(View.VISIBLE);
        } else if ("grid_defender".equals(personnelType)) {
            // 网格联防员：不显示详情卡片
            tvWorkExperiences.setVisibility(View.GONE);
            tvAwards.setVisibility(View.GONE);
            tvFamilyMembers.setVisibility(View.GONE);
